...
struct LoggerTest : public Testing::TestGroup<LoggerTest> {
	explicit LoggerTest() : TestGroup(Testing::SingleThreaded | Testing::ProcessPerTest) {
		RegisterTest(stringMessages);
		RegisterTest(parameterisedMessages);
		RegisterTest(loggerMacros);
		RegisterTest(getConfigurationCall);
		RegisterTest(globalNamespace);
		RegisterTest(unconfiguredRootNamespace);
		RegisterTest(configuredNamespaceWithoutFormat);
		RegisterTest(loggingWithLineAndFileName);
		RegisterTest(functionBasedLogging);
		RegisterTest(flushing);
		RegisterTest(customLoggingStream);
		RegisterTest(resetLoggingSystem);
	}
	...
	. {
		...
		const std::vector<std::string> expectedContains = {
			...
			, ": - [LoggerTest::parameterisedMessages] INFO - com.borasoftware - A three 1 parameter 2.000000 message 3"
			, ": - [LoggerTest::parameterisedMessages] INFO - com.borasoftware - 1, 2.000000, 3, 4.000000"
		};
	...
	. {
		...
		const std::vector<std::string> expectedContains = {
			...
			, " - [LoggerTest::loggerMacros] INFO - com.borasoftware - A three 1 parameter 2.000000 message 3"
			, " - [LoggerTest::loggerMacros] INFO - com.borasoftware - 1, 2.000000, 3, 4.000000"
