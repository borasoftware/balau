...
struct FileServingHttpWebAppTest : public Testing::TestGroup<FileServingHttpWebAppTest> {
	FileServingHttpWebAppTest() {
		...
		//RegisterTest(getFile);
		//RegisterTest(getNotFoundFile);
		//RegisterTest(headFile);
		//RegisterTest(headNotFoundFile);
		//RegisterTest(getFile1000Serial);
		//RegisterTest(getFile1000Parallel);
		//RegisterTest(getFiles);
	}
	...
	template <typename ResponseT> static void assertResponse(const ResponseT & response,
		...
		AssertThat(reason.to_string(), is(expectedReason));
