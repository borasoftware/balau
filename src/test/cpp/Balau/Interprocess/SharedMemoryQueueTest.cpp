...
struct SharedMemoryQueueTest : public Testing::TestGroup<SharedMemoryQueueTest> {
	SharedMemoryQueueTest() {
		RegisterTest(singleBufferSPST);
		RegisterTest(singleBufferSISO);
		RegisterTest(singleBufferSIMO);
		RegisterTest(singleBufferMISO);
		RegisterTest(singleBufferMIMO);
		RegisterTest(multipleBufferSPST);
		RegisterTest(multipleBufferSISO);
		RegisterTest(multipleBufferMISO);
		RegisterTest(multipleBufferPending);
	}
	...
	static void runSPST(SharedMemoryQueue<SMT> & queue) {
	...
	static void runSISO(SharedMemoryQueue<SMT> & queue) {
