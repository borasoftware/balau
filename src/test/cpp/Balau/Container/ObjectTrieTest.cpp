...
struct ObjectTrieTest : public Testing::TestGroup<ObjectTrieTest> {
	ObjectTrieTest() {
		RegisterTest(uIntTrieBuild);
		RegisterTest(uIntTrieCopy);
		RegisterTest(uIntTreeDepthIterate);
		RegisterTest(uIntTreeDepthIterateForLoop);
		RegisterTest(uIntTreeBreadthIterate);
		RegisterTest(fluentBuild);
	}
	...
	static void updateUIntTrie(UIntTrie & trie, unsigned int dataCounter = 0) {
	...
	static void assertUIntTrie(const UIntTrie & trie, unsigned int dataCounter = 0) {
