...
struct Hashing final {
	...
	private: static std::unique_ptr<std::ifstream> getFileStream(const Resource::File & file) {
		if (!file.exists()) {
			ThrowBalauException(Exception::FileNotFoundException, file);
		}

		auto stream = std::make_unique<std::ifstream>(file.getEntry().path(), std::ios::in | std::ios::binary);
