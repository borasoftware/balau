...
template <typename T, typename D> class RandomNumberGenerator final {
	...
	///
	public: RandomNumberGenerator(T lower = std::numeric_limits<T>::min(), T upper = std::numeric_limits<T>::max())
	...
	///
	public: RandomNumberGenerator(unsigned int seed, T lower = std::numeric_limits<T>::min(), T upper = std::numeric_limits<T>::max())
		: generator(seed)
		, distribution(D(lower, Impl::inclusivise(upper))) {}
