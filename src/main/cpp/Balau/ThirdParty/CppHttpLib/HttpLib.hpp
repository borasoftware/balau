...
#define CPPHTTPLIB_ZLIB_SUPPORT BALAU_ENABLE_ZLIB
...
class ClientImpl {
	...
	virtual std::unique_ptr<ClientImpl> clone() const;
	...
	// Currently required for boost streams.. delete once boost dependency is removed.
	ClientImpl(const ClientImpl & copy)
		: host_(copy.host_)
		, port_(copy.port_)
		, host_and_port_(copy.host_and_port_)
		, socket_(Socket()) // Not open
		, socket_requests_in_flight_(copy.socket_requests_in_flight_)
		, socket_should_be_closed_when_request_is_done_(copy.socket_should_be_closed_when_request_is_done_)
		, default_headers_(copy.default_headers_)
		, client_cert_path_(copy.client_cert_path_)
		, client_key_path_(copy.client_key_path_)
		, connection_timeout_sec_(copy.connection_timeout_sec_)
		, connection_timeout_usec_(copy.connection_timeout_usec_)
		, read_timeout_sec_(copy.read_timeout_sec_)
		, read_timeout_usec_(copy.read_timeout_usec_)
		, write_timeout_sec_(copy.write_timeout_sec_)
		, write_timeout_usec_(copy.write_timeout_usec_)
		, basic_auth_username_(copy.basic_auth_username_)
		, basic_auth_password_(copy.basic_auth_password_)
		, bearer_token_auth_token_(copy.bearer_token_auth_token_)
		#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
		, digest_auth_username_(copy.digest_auth_username_)
		, digest_auth_password_(copy.digest_auth_password_)
		#endif
		, keep_alive_(copy.keep_alive_)
		, follow_location_(copy.follow_location_)
		, url_encode_(copy.url_encode_)
		, address_family_(copy.address_family_)
		, tcp_nodelay_(copy.tcp_nodelay_)
		, socket_options_(copy.socket_options_)
		, compress_(copy.compress_)
		, decompress_(copy.decompress_)
		, interface_(copy.interface_)
		, proxy_host_(copy.proxy_host_)
		, proxy_port_(copy.proxy_port_)
		, proxy_basic_auth_username_(copy.proxy_basic_auth_username_)
		, proxy_basic_auth_password_(copy.proxy_basic_auth_password_)
		, proxy_bearer_token_auth_token_(copy.proxy_bearer_token_auth_token_)
		#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
		, proxy_digest_auth_username_(copy.proxy_digest_auth_username_)
		, proxy_digest_auth_password_(copy.proxy_digest_auth_password_)
		#endif
		#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
		, server_certificate_verification_(copy.server_certificate_verification_)
		#endif
	{}
...
class Client {
	...
	#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
	Client(std::unique_ptr<ClientImpl> cli, bool is_ssl) : cli_(std::move(cli)), is_ssl_(is_ssl) {}
	#else
	Client(std::unique_ptr<ClientImpl> cli) : cli_(std::move(cli)) {}
	#endif
};
...
class SSLClient : public ClientImpl {
	...
	std::unique_ptr<ClientImpl> clone() const override;
	...
	private:
	void setupCtx(const std::string & client_cert_path, const std::string & client_key_path);
	...
	// Currently required for boost streams.. delete once boost dependency is removed.
	SSLClient(const SSLClient & copy)
		: ClientImpl(copy)
		, ctx_(nullptr)
		, host_components_(copy.host_components_)
		, ca_cert_file_path_(copy.ca_cert_file_path_)
		, ca_cert_dir_path_(copy.ca_cert_dir_path_)
		, verify_result_(copy.verify_result_) {
			setupCtx(ca_cert_file_path_, ca_cert_dir_path_);
		}
...
inline std::unique_ptr<ClientImpl> ClientImpl::clone() const {
	return std::unique_ptr<ClientImpl>(new ClientImpl(*this));
}
...
inline void SSLClient::setupCtx(const std::string & client_cert_path, const std::string & client_key_path) {
...
inline SSLClient::SSLClient(const std::string & host,
	...
	setupCtx(client_cert_path, client_key_path);
}
...
inline std::unique_ptr<ClientImpl> SSLClient::clone() const {
	return std::unique_ptr<ClientImpl>(new SSLClient(*this));
}
...
// TODO improve.. remove manually constructed string
inline Client::Client(const std::string & host,
                      int port,
                      bool isSsl,
                      const std::string & client_cert_path,
                      const std::string & client_key_path)
	: Client(((isSsl ? "https://" : "http://") + host + ":" + std::to_string(port)).c_str(), client_cert_path, client_key_path) {}

inline std::unique_ptr<Client> Client::clone() const {
	#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
		return std::unique_ptr<Client>(new Client(cli_->clone(), is_ssl_));
	#else
		return std::unique_ptr<Client>(new Client(cli_->clone()));
	#endif
}
