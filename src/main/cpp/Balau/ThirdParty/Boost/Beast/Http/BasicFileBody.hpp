...
#ifndef BALAU_BOOST_BEAST_HTTP_BASIC_FILE_BODY_HPP
#define BALAU_BOOST_BEAST_HTTP_BASIC_FILE_BODY_HPP
...
namespace Balau::Boost::Beast::Http {

// Throws a network exception instead of segfaulting.
inline void BALAU_BOOST_ASSERT(bool condition, const char * message) {
	if (!condition) {
		ThrowBalauException(Exception::NetworkException, message);
	}
}
...
struct basic_file_body {
	...
	static_assert(boost::beast::is_file<File>::value, "File requirements not met");
...
writer(boost::beast::http::header<isRequest, Fields> & h, value_type & b)
	...
	BALAU_BOOST_ASSERT(body_.file_.is_open(), "The file is not open (was the file deleted?).");
...
basic_file_body<File>::
reader::
init(boost::optional<std::uint64_t> const & content_length, boost::beast::error_code & ec) {
...
template <class ConstBufferSequence>
std::size_t
basic_file_body<File>::reader::put(ConstBufferSequence const & buffers, boost::beast::error_code & ec) {
...
template <class File>
void
basic_file_body<File>::reader::finish(boost::beast::error_code & ec) {
...
std::ostream &
operator <<(std::ostream & os, boost::beast::http::message<isRequest, basic_file_body<File>, Fields> const & msg) = delete;

#endif

} // Balau::Boost::Beast::Http
